...
def check_winner(masks: np.array, boards: np.array) -> int:
    n_boards, n_rows, n_cols = masks.shape
    for i in range(n_boards):
        for row in range(n_rows):
            if all(masks[i, row, :] == 0):
                return int(np.sum(masks[i] * boards[i]))
...
def main():
    ...
    for number in numbers:
        ...
        winner_sum = check_winner(masks, boards)
        if winner_sum != -1:
            print(f'Final score: {winner_sum * number}')
            break
